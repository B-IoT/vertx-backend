...
fun getItemsWithAC(itemsTable: String, beaconDataTable: String) =
  "SELECT DISTINCT ON (I.id) * FROM $itemsTable I LEFT JOIN $beaconDataTable D ON I.beacon = D.mac WHERE I.accessControlString SIMILAR TO '$1(:|$)' ORDER BY I.id, D.time DESC"
...
fun getItemsWithCategoryWithAC(itemsTable: String, beaconDataTable: String) =
  "SELECT DISTINCT ON (I.id) * FROM $itemsTable I LEFT JOIN $beaconDataTable D ON I.beacon = D.mac WHERE I.category=$1 AND I.accessControlString SIMILAR TO '$2(:|$)'  ORDER BY I.id, D.time DESC"
